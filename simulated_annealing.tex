...
\begin{document}
	...
	\item É utilizada a função de redução, sendo esta geométrica compreendida por: $T_i = CT_{i-1}$, onde $C<1$, constante e se encontra usualmente entre 0.75 e 0.95;
...
sa_weibull3 <- function(
  ...
  while (T > Tf) {
    for (i in 1:L) {
      ...
      history <- rbind(history, data.frame(eval = eval_count, 
      ll = curr_ll))
    }
    T <- cooling_rate * T
  }
